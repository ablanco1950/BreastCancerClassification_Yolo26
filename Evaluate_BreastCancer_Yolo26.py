...
"""
 ...
 Alfonso Blanco García , jun 2026
"""
######################################################################
# https://zenodo.org/records/10519652
# dataset breastmnist_128.npz
######################################################################
...
#model = YOLO("runs\\classify\\train\\weights\\best.pt")  # Ensure you have this model file or let Ultralytics auto-download it
...
classes, c_to_idx = find_classes('Dir_BreastCancer_Yolo26/test')
...
def loadimagesTest():
    imgpath = "Dir_BreastCancer_Yolo26\\test\\"
...
TotalFailures=0
TotalRejected=0
with open( "BreastImagesRejected.txt" ,"w") as  w:
    ...
    for i in range(len(TabImagePath)):
        ...
        top_class_index, NameBreastCancerPredicted,confidence =classify_image(TabImagePath[i])
        ...
        if confidence < CONF:
            ...
            #NameBreastCancerTrue=Y_test_confidence[i]
            #lineaw.append(NameBreastCancerTrue)
            #lineaw.append(NameBreastCancerPredicted)
        ...
        TabPredictions.append(NameBreastCancerPredicted)
        ...
        if NameBreastCancerPredicted!=NameBreastCancerTrue:
            TotalFailures=TotalFailures + 1
            print("ERROR " + imageName_test[i]+ " is assigned Model " +  NameBreastCancerPredicted
                  + "  True Model " +  NameBreastCancerTrue)
                  
        else:
            print("HIT " + imageName_test[i]+ " is assigned model " +   NameBreastCancerPredicted)
